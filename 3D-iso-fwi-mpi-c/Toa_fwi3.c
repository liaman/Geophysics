...
/*******************************************************/
        char FN1[250]={"shot5050layer.dat"};
	  char FN2[250]={"vel505050.dat"};
        char FN3[250]={"shot_initial1.dat"};
        char FN4[250]={"multi_gradient1.dat"};
        char FN5[250]={"cg_gradient1.dat"};

        char FN6[250]={"Vel_Recording1.dat"};

        char FN7[250]={"vel505050layer.dat"};

         

/*******************************************************/     
        
	int i,j,k,is,nx,ny,nz,nt,vnx,vny,vnz,i_start,i_end,l;
...
/*******************************************************/
        nx=50;          npd=30;   tmax=1.6;
        ny=50;          favg=60;  pfac=10000.0;
	  nz=50;         
...
/***********************************  back propagation   **********************************/  
/******************************************************************************************/   
        back_grad(myid,nx,ny,nz,vnx,vny,vnz,nt,npd,dx,dy,dz,vdx,vdy,vdz,favg,tmax,dt,dtout,FN7,ns_sxd,
                 xs_sxd[is-1],ys_sxd[is-1],zs_sxd,is,p_com,p_initial,p_down,p_left,p_right,p_front,p_back,g0);
...
void back_grad(int myid,int nx,int ny,int nz,int vnx,int vny,int vnz,int nt,int npd,float dx,float dy,float dz,
               ...
               float ***p_left,float ***p_right,float ***p_front,float ***p_back,float ***g)
{
...
/******************************************** the snap dat output   ******************/
           ...
           {
              FILE *fpsnap;
              fpsnap=fopen("snapf_it150.dat","wb");
              for(j=0;j<ny;j++)
	      {
		 for(i=0;i<nx;i++)
		 {
           ...
           {
              FILE *fpsnap;
              fpsnap=fopen("snapf_it250.dat","wb");
              for(j=0;j<ny;j++)
	      {
		 for(i=0;i<nx;i++)
		 {
           ...
           {
              FILE *fpsnap;
              fpsnap=fopen("snapf_it300.dat","wb");
              for(j=0;j<ny;j++)
	      {
		 for(i=0;i<nx;i++)
		 {
