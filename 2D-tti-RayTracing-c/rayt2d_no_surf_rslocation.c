...
void main(int argc, char **argv)
{
	...
	FILE *vfp, *tfp, *jpfp, *pvfp=NULL, *tvfp=NULL, *csfp=NULL;
        ...
        char vfile[250]={"vel_600_300.dat"};
	...
	amax = 180.; //maximum emergence angle出射角
	...
	npv = 1; //flag of computing quantities for velocity analysis标记速度分析的计算量
	...
	if(npv){ 
	    ...
	    pvfp = fopen(pvfile,"r"); 
		tvfile = "tvfile";
	    tvfp = fopen(tvfile,"w");       //走时表变化的输出文件
		csfile = "csfile";
	    csfp = fopen(csfile,"w");
	}
	...
	for(ixs=ixs0,xs=fxs+ixs0*dxs;ixs<nxs;ixs++,xs+=dxs){
		...
		raytr.xs = xs;		raytr.zs = szi[ixs];raytr.zs = 1.0;
		...
		if(npv) {
			fwrite(tv,sizeof(float),nxo*nzo,tvfp);//走时表变化的输出文件
