...
main(int argc,char *argv[])
{
        ...
        void model_step(int nx,int nz,int vnx,int vnz,int nt,int npd,float dx,float dz,
	...
	int i,j,k,is,nx,nz,nt,ntout,nttt=0,vnx,vnz,i_start,i_end,mz,vmz,ncdp;
...
/*************parameter****************/  
          ...
          nx=500;         npd=50;   tmax=2;
	    nz=200;         favg=30;  pfac=10000.0;
 	                    dx=5.0;   
	    vnx=500;        dz=5.0;   
	    vnz=200;        vdx=5.0;
	                    vdz=5.0;
...
/*        nx=100;         npd=50;   tmax=0.8;
	    nz=90;         favg=20;  pfac=1000.0;
 	                dx=10.0;   
	    vnx=100;        dz=10.0;   
	    vnz=90;        vdx=10.0;
	    nt=801;        vdz=10.0;
...
/***************************************/
        ...
        if(myid==0)
	{
	  printf("1. the model is start  !  \n");
        ...
        model_step(nx,nz,vnx,vnz,nt,npd,dx,dz,vdx,vdz,favg,tmax,dt,dtout,
                 pfac,FN1,ns_sxd,ds_sxd,fs_sxd,zs_sxd,is,p_cal,myid);   
         if(myid==0)
	{
	  printf("2. start mute the direct wave  !\n");
	}
       // model_step(nx,mz,vnx,vmz,nt,npd,dx,dz,vdx,vdz,favg,tmax,dt,dtout,
      //           pfac,FN4,ns_sxd,ds_sxd,fs_sxd,zs_sxd,is,p_m,myid);//++++++  
	...
	{ 
	  for(j=0;j<nt;j++)
	  {
        ...
        fseek(fp3,(is-1)*nx*ntout*4L,0);
	for(i=0;i<nx;i++)
	{ 
	  for(j=0;j<ntout;j++)
	  {
          nttt=(int)(dtout/dt);
	    fwrite(&p_cal[i][j*nttt],4L,1,fp3);
	  }
	}        
      fseek(fp5,(is-1)*ncdp*ntout*4L,0);
	/*for(i=fs_sxd+(is-1)*ds_sxd-(ncdp-1);i<fs_sxd+(is-1)*ds_sxd+(ncdp-1);i+=2)
	{ 
	  for(j=0;j<ntout;j++)
	  {
          nttt=(int)(dtout/dt);
	    fwrite(&p_cal[i][j*nttt],4L,1,fp5);
	  }
	}           printf("----------4\n");*/
               ...
               int is,float **p_cal,int myid)
{
    ...
    void update_vel(int nx,int nz,int npd,int mm,float dt,float dx,float dz,
                  ...
                  float **rho,float c[],float *coffx1,float *coffx2,
                  float *coffz1,float *coffz2);
    void update_txx(int nx,int nz,float dt,float dx,float dz,int mm,float **u0,float **w0,
                  float **txx0,float **u1,float **w1,float **txx1,float **s,float **vp,
                  float c[],int npd,float **tx1,float **tx0,float **tz1,float **tz0,
	...
	void initial_coffe(float dt,float d0,int nx,int nz,float *coffx1,float *coffx2,
                  float *coffz1,float *coffz2,float *acoffx1,float *acoffx2,float *acoffz1,
                  float *acoffz2,int npd);	
    void freea(float **a,int m);
...
/***********************************************/  
    ...
    { 
	  ...
	  ptsource(pfac,fs_sxd,zs_sxd,nx,nz,dt,t,favg,s,wtype,pi,npd,is,ds_sxd);
        update_vel(nx,nz,npd,mm,dt,dx,dz,u0,w0,txx0,u1,w1,txx1,rho,c,
                 coffx1,coffx2,coffz1,coffz2);
        update_txx(nx,nz,dt,dx,dz,mm,u0,w0,txx0,u1,w1,txx1,s,vp,c,npd,
                tx1,tx0,tz1,tz0,acoffx1,acoffx2,acoffz1,acoffz2);
	...
	/*	if((it%50==0)&&(is==1)&&(myid==0))  
                 {

                    fseek(fp33,(it/20)*nx*nz*4L,0);
                    for(i=0;i<nx;i++)
		    {
                        for(j=0;j<nz;j++)
                        {
                            fwrite(&txx1[i+npd][j+npd],4L,1,fp33);
                        }
                    }
       ...
       if(mm>4)//*****************//
       {
         for(i=mm;i<=(2*npd+nz-mm-1);i++)
		   for(j=mm;j<=(2*npd+nx-mm-1);j++)
           {
             uu=0.0;
			 for(im=0;im<mm;im++)
               uu+=c[im]*(txx0[i][j+im+1]-txx0[i][j-im]);
             u1[i][j]=coffx2[j]*u0[i][j]-coffx1[j]*dtx*rho[i][j]*uu;
           }
		   ...
		   for(i=mm;i<=(2*npd+nz-mm-1);i++)
			{
             ww=0.0;
             for(im=0;im<mm;im++)
                ww+=c[im]*(txx0[i+im+1][j]-txx0[i-im][j]);
             w1[i][j]=coffz2[i]*w0[i][j]-coffz1[i]*dtz*rho[i][j]*ww;
      ...
      if(mm>4)//***************//
		 for(i=mm;i<=(2*npd+nz-mm-1);i++)
		   for(j=mm;j<=(2*npd+nx-mm-1);j++)
		   {
		        ...
		        for(im=0;im<mm;im++)
                         {
                   xx+=c[im]*(u1[i][j+im]-u1[i][j-im-1]);
                   zz+=c[im]*(w1[i+im][j]-w1[i-im-1][j]);
                         }
 
		    	tx1[i][j]=acoffx2[j]*tx0[i][j]-acoffx1[j]*vp[i][j]*dtx*xx;

                tz1[i][j]=acoffz2[i]*tz0[i][j]-acoffz1[i]*vp[i][j]*dtz*zz;
...
void zero3d(int nx,int nz,int nt,int npd,float **u0,float **w0,float **txx0,float **u1,float **w1,float **txx1,float **vp,float **rho)
{
		 int i,j,k;
