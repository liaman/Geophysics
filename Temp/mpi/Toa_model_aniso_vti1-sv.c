...
main(int argc,char *argv[])
{
        ...
        void model(int nx,int nz,int vnx,int vnz,int nt,int npd,float dx,float dz,
                   ...
                   int is,float **p_cal,float deta,float epsilu);
...
//	char FN3[250]={"shot_mar_350X4001.dat"};
	...
	float dx,dz,vdx,vdz,tmax,dt,dtout,pfac,favg,deta,epsilu;
        ...
        if(myid==0)
	{
	  printf("the model is start    \n");
        ...
        model(nx,nz,vnx,vnz,nt,npd,dx,dz,vdx,vdz,favg,tmax,dt,dtout,pfac,FN1,
              ns_sxd,ds_sxd,fs_sxd,zs_sxd,is,p_cal,deta,epsilu);        
           ...
           float **rho,float c[],float *coffx1,float *coffx2,float *coffz1,float *coffz2)
{
		 ...
		 for(i=mm;i<=(2*npd+nx-mm-1);i++)
		 {
			 for(j=mm;j<=(2*npd+nz-mm-1);j++)
			 {
                           ...
                           wq1[i][j]=coffz2[j]*wq0[i][j]-coffz1[j]*dtz*rho[i][j]*(c[0]*(tzz0[i][j+1]-tzz0[i][j])+c[1]*(tzz0[i][j+2]-tzz0[i][j-1])+c[2]*(tzz0[i][j+3]-tzz0[i][j-2])+c[3]*(tzz0[i][j+4]-tzz0[i][j-3]));
		 ...
		 { 
			 for(i=mm;i<=(2*npd+nx-mm-1);i++)
			 {
			   up1[i][j]=coffx2[i]*up0[i][j]-coffx1[i]*dtx*rho[i][j]*(c[0]*(txx0[i+1][j]-txx0[i][j])+c[1]*(txx0[i+2][j]-txx0[i-1][j])+c[2]*(txx0[i+3][j]-txx0[i-2][j])+c[3]*(txx0[i+4][j]-txx0[i-3][j]));
			 }
		 }
            ...
            float deta,float epsilu,int xsn,int ds_sxd,int zsn,int is)
{
		 ...
		 for(i=mm;i<=(2*npd+nx-mm-1);i++)
		 {
		    for(j=mm;j<=(2*npd+nz-mm-1);j++)
		    {
                        ix=i-ixs;
                        iz=j-izs;
                        if((ix*ix+iz*iz)<=sc*sc)
                        {
                           if((ix*ix+iz*iz)<=(sc*sc/16))
                           { 
                              epsilu1=0.0;
                              deta1=0.0;
                           }
                          ...
                          /*    epsilu1=(1.5*( pow((ix*ix+iz*iz),0.5) / (1.0*sc) )-0.5)*epsilu;
                              deta1=(1.5*( pow((ix*ix+iz*iz),0.5) / (1.0*sc) )-0.5)*deta;  */     //gradient window
                              epsilu1=0.5*(1-cos(pi*( (pow((ix*ix+iz*iz),0.5)-sc/4.0)*4.0/(sc*3.0-1)) ))*epsilu;
                              deta1=0.5*(1-cos(pi*( (pow((ix*ix+iz*iz),0.5)-sc/4.0)*4.0/(sc*3.0-1)) ))*deta;   
                           }

                           tpx1[i][j]=acoffx2[i]*tpx0[i][j]-acoffx1[i]*vp[i][j]*(1+2*epsilu1)*dtx*(c[0]*(up1[i][j]-up1[i-1][j])+c[1]*(up1[i+1][j]-up1[i-2][j])+c[2]*(up1[i+2][j]-up1[i-3][j])+c[3]*(up1[i+3][j]-up1[i-4][j]));	
 
                           tpz1[i][j]=acoffz2[j]*tpz0[i][j]-acoffz1[j]*vp[i][j]*(pow((1+2*deta1),0.5))*dtz*(c[0]*(wq1[i][j]-wq1[i][j-1])+c[1]*(wq1[i][j+1]-wq1[i][j-2])+c[2]*(wq1[i][j+2]-wq1[i][j-3])+c[3]*(wq1[i][j+3]-wq1[i][j-4]));	         

                           tqx1[i][j]=acoffx2[i]*tqx0[i][j]-acoffx1[i]*vp[i][j]*(pow((1+2*deta1),0.5))*dtx*(c[0]*(up1[i][j]-up1[i-1][j])+c[1]*(up1[i+1][j]-up1[i-2][j])+c[2]*(up1[i+2][j]-up1[i-3][j])+c[3]*(up1[i+3][j]-up1[i-4][j]));

                           tqz1[i][j]=acoffz2[j]*tqz0[i][j]-acoffz1[j]*vp[i][j]*dtz*(c[0]*(wq1[i][j]-wq1[i][j-1])+c[1]*(wq1[i][j+1]-wq1[i][j-2])+c[2]*(wq1[i][j+2]-wq1[i][j-3])+c[3]*(wq1[i][j+3]-wq1[i][j-4]));	
                        ...
                        {           
                           tpx1[i][j]=acoffx2[i]*tpx0[i][j]-acoffx1[i]*vp[i][j]*(1+2*epsilu)*dtx*(c[0]*(up1[i][j]-up1[i-1][j])+c[1]*(up1[i+1][j]-up1[i-2][j])+c[2]*(up1[i+2][j]-up1[i-3][j])+c[3]*(up1[i+3][j]-up1[i-4][j]));	
 
                           tpz1[i][j]=acoffz2[j]*tpz0[i][j]-acoffz1[j]*vp[i][j]*(pow((1+2*deta),0.5))*dtz*(c[0]*(wq1[i][j]-wq1[i][j-1])+c[1]*(wq1[i][j+1]-wq1[i][j-2])+c[2]*(wq1[i][j+2]-wq1[i][j-3])+c[3]*(wq1[i][j+3]-wq1[i][j-4]));	         

                           tqx1[i][j]=acoffx2[i]*tqx0[i][j]-acoffx1[i]*vp[i][j]*(pow((1+2*deta),0.5))*dtx*(c[0]*(up1[i][j]-up1[i-1][j])+c[1]*(up1[i+1][j]-up1[i-2][j])+c[2]*(up1[i+2][j]-up1[i-3][j])+c[3]*(up1[i+3][j]-up1[i-4][j]));
