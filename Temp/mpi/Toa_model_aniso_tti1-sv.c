...

{
	...
	void ptsource(float pfac,float xsn,float zsn,int nx,int nz,float dt,float t,
        ...
        void update_stress(int nx,int nz,float dt,float dx,float dz,int mm,
                   ...
                   float deta,float epsilu,float ceta,int xsn,int ds_sxd,int zsn,int is);
    ...
    for(it=0,t=0.0;it<nt;it++,t+=dt)
    {
	  ...
	  ptsource(pfac,fs_sxd,zs_sxd,nx,nz,dt,t,favg,s,wtype,pi,npd,is,ds_sxd);
          update_vel(nx,nz,npd,mm,dt,dx,dz,up0,wp0,uq0,wq0,txx0,tzz0,
                     up1,wp1,uq1,wq1,txx1,tzz1,rho,c,
                     coffx1,coffx2,coffz1,coffz2,ceta);
          update_stress(nx,nz,dt,dx,dz,mm,up0,wp0,uq0,wq0,txx0,tzz0,
                        ...
                        deta,epsilu,ceta,fs_sxd,ds_sxd,zs_sxd,is);
...
void update_vel(int nx,int nz,int npd,int mm,float dt,float dx,float dz,
               ...
               float **up1,float **wp1,float **uq1,float **wq1,float **txx1,float **tzz1,
               float **rho,float c[],
               float *coffx1,float *coffx2,float *coffz1,float *coffz2,float ceta)
{
		 int ii,i,j;
               ...
               float deta,float epsilu,float ceta,int xsn,int ds_sxd,int zsn,int is)
{
		 ...
		 for(i=mm;i<=(2*npd+nx-mm-1);i++)
		 {
		    for(j=mm;j<=(2*npd+nz-mm-1);j++)
		    {
                     ...
                     //  dp/dt=du/dx-du/dz
                           tpx1[i][j]=acoffx2[i]*acoffz2[j]*tpx0[i][j]-acoffx1[i]*acoffz1[j]*vp[i][j]*(1+2*epsilu1)*( cos(ceta)*dtx*(c[0]*(up1[i][j]-up1[i-1][j])+c[1]*(up1[i+1][j]-up1[i-2][j])+c[2]*(up1[i+2][j]-up1[i-3][j])+c[3]*(up1[i+3][j]-up1[i-4][j])) - sin(ceta)*dtz*(c[0]*(up1[i][j]-up1[i][j-1])+c[1]*(up1[i][j+1]-up1[i][j-2])+c[2]*(up1[i][j+2]-up1[i][j-3])+c[3]*(up1[i][j+3]-up1[i][j-4])) );

                           tpz1[i][j]=acoffz2[j]*acoffx2[i]*tpz0[i][j]-acoffz1[j]*acoffx1[i]*vp[i][j]*(pow((1+2*deta1),0.5))*( cos(ceta)*dtz*(c[0]*(wq1[i][j]-wq1[i][j-1])+c[1]*(wq1[i][j+1]-wq1[i][j-2])+c[2]*(wq1[i][j+2]-wq1[i][j-3])+c[3]*(wq1[i][j+3]-wq1[i][j-4])) + sin(ceta)*dtx*(c[0]*(wq1[i][j]-wq1[i-1][j])+c[1]*(wq1[i+1][j]-wq1[i-2][j])+c[2]*(wq1[i+2][j]-wq1[i-3][j])+c[3]*(wq1[i+3][j]-wq1[i-4][j])) );

                           tqx1[i][j]=acoffx2[i]*acoffz2[j]*tqx0[i][j]-acoffx1[i]*acoffz1[j]*vp[i][j]*(pow((1+2*deta1),0.5))*( cos(ceta)*dtx*(c[0]*(up1[i][j]-up1[i-1][j])+c[1]*(up1[i+1][j]-up1[i-2][j])+c[2]*(up1[i+2][j]-up1[i-3][j])+c[3]*(up1[i+3][j]-up1[i-4][j])) - sin(ceta)*dtz*(c[0]*(up1[i][j]-up1[i][j-1])+c[1]*(up1[i][j+1]-up1[i][j-2])+c[2]*(up1[i][j+2]-up1[i][j-3])+c[3]*(up1[i][j+3]-up1[i][j-4])) );
