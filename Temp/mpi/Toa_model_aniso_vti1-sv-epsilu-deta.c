...
void model_vti(int nx,int nz,int vnx,int vnz,int nt,int npd,float dx,float dz,
             ...
             int is,float **p_cal,float **deta,float **epsilu,float Circle_Radius,
...
/******************* dat document **********************/
	  ...
	  char FN2[250]={"shot.dat"};

/***************** parameters **************************/

        nx=300;         npd=50;      tmax=2.0;
...
/********************IS Loop start**************/
   for(is=1+myid;is<=ns_sxd;is=is+numprocs)	
    {     
      ...
      model_vti(nx,nz,vnx,vnz,nt,npd,dx,dz,vdx,vdz,favg,tmax,dt,dtout,pfac,FN1,
            ns_sxd,ds_sxd,fs_sxd,zs_sxd,is,p_cal,deta,epsilu,Circle_Radius,
...
void read_vrho(char FN1[],int nx,int nz,float **vv,float **rho0,int npd);
...
/********************************************************/
       read_vrho(FN1,vnx,vnz,vp0,rho0,npd); 
