...
main(int argc,char *argv[])
{
        ...
        void model_step(int nx,int nz,int vnx,int vnz,int nt,int npd,float dx,float dz,float vdx,float vdz,float favg,
                        float tmax,float dt,float dtout,float pfac,char FN1[],int ns_sxd,int ds_sxd,int fs_sxd,int zs_sxd,int is,float **p_cal);
...
//	char FN3[250]={"shot_mar_350X4001.dat"};
        ...
        char FN3[250]={"layer_shot.dat"};

        char FN4[250]={"vel_for_mute_by_PRP_model.dat"};
        
        FILE *fp3;
        fp3=fopen(FN3,"wb");
        
	int i,j,k,is,nx,nz,nt,vnx,vnz,i_start,i_end,mz,vmz;
...
/***************************************/
        ...
        if(myid==0)
	{
	  printf("1. the model is start  !  \n");
        ...
        model_step(nx,nz,vnx,vnz,nt,npd,dx,dz,vdx,vdz,favg,tmax,dt,dtout,pfac,FN1,ns_sxd,ds_sxd,fs_sxd,zs_sxd,is,p_cal);   
         if(myid==0)
	{
	  printf("2. start mute the direct wave  !\n");
	}
        model_step(nx,mz,vnx,vmz,nt,npd,dx,dz,vdx,vdz,favg,tmax,dt,dtout,pfac,FN4,ns_sxd,ds_sxd,fs_sxd,zs_sxd,is,p_m);//++++++  
...
void model_step(int nx,int nz,int vnx,int vnz,int nt,int npd,float dx,float dz,float vdx,float vdz,float favg,float tmax,float dt,float dtout,float pfac,char FN1[],int ns_sxd,int ds_sxd,int fs_sxd,int zs_sxd,int is,float **p_cal)

{


	void cal_c(int mm,float c[]);
	void ptsource(float pfac,float xsn,float zsn,int nx,int nz,float dt,float t,float favg,float **s,int wtype,float pi,int npd,int is,int ds_sxd);
        void ptsource_wavelet(float pfac,float xsn,float zsn,int nx,int nz,float dt,float t,float favg,float **s,int wtype,float pi,int npd,int is,int ds_sxd,int nt,int it);//++++++++++++++
        void update_vel(int nx,int nz,int npd,int mm,float dt,float dx,float dz,float **u0,float **w0,float **txx0,float **u1,float **w1,float **txx1,float **rho,float c[],float *coffx1,float *coffx2,float *coffz1,float *coffz2);
        void update_txx(int nx,int nz,float dt,float dx,float dz,int mm,float **u0,float **w0,float **txx0,float **u1,float **w1,float **txx1,float **s,float **vp,float c[],int npd,float **tx1,float **tx0,float **tz1,float **tz0,float *acoffx1,float *acoffx2,float *acoffz1,float *acoffz2); 
...

{
	    ...
	    tdelay=1.0/favg;
           ...
           {
            //source=get_source(ts,favg,wtype);   
            wavelet=read_wavelet("wavelet_lowpass.dat",nt,it);//++++++++++++++++++++
	    ixs = (int)(fs+0.5)+npd-1;
            izs = (int)(zsn+0.5)+npd-1;
