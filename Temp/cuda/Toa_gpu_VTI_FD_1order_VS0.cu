...
__global__ void add_source(float pfac,float xsn,float zsn,int nx,int nz,int nnx,int nnz,float dt,float t,
                        float favg,int wtype,int npml,int is,int ds,float *P,float *Q,float *PQ)
                           ...
                           float *coffx1,float *coffx2,float *coffz1,float *coffz2)
{
	...
	float dtx,dtz,xx,zz,xz,zx;
        ...
        iz=id%nnz;
		 ...
		 dtx=dt/dx;
		 dtz=dt/dz;
               if(id>=mm+1&&id<nnx*nnz-mm-1)
                 {
                   if(ix>=mm+1&&ix<(nnx-mm-1)&&iz>=mm+1&&iz<(nnz-mm-1))
                    {
                     xx=0.0;
                     xz=0.0;
                     zz=0.0;
	             ...
	             for(im=0;im<mm;im++)
                    ...
                    /*    xx+=c[im]*( P[id+(im+1)*nnz] -   P[id-im*nnz]);
                        xz+=c[im]*(PQ[id+im+1]       -  PQ[id-im]);
                        zz+=c[im]*( Q[id+im+1]       -   Q[id-im]);
                        zx+=c[im]*(PQ[id+(im+1)*nnz] -  PQ[id-im*nnz]); */

                        xx+=c[im]*( P[id+(im+1)*nnz] -   P[id-im*nnz]);
                        xz+=c[im]*(PQ[id+im]       -  PQ[id-im-1]);
                        zz+=c[im]*( Q[id+im+1]       -   Q[id-im]);
                        zx+=c[im]*(PQ[id+(im)*nnz] -  PQ[id-(im+1)*nnz]);
                      }
                     u1[id]=coffx2[ix]*coffz2[iz]*u0[id]-coffx1[ix]*coffz1[iz]*(dtx*xx+dtz*xz);
                     w1[id]=coffx2[ix]*coffz2[iz]*w0[id]-coffx1[ix]*coffz1[iz]*(dtz*zz+dtx*zx);
                   }
                 }
}
/*******************func***********************/
__global__ void update_stress(int nx,int nz,int nnx,int nnz,float dt,float dx,float dz,
                           float *u1,float *w1,float *P,float *Q,float *PQ,float *vp,float *vs,int npml,
                           float *px1,float *px0,float *pz1,float *pz0,
                           float *qx1,float *qx0,float *qz1,float *qz0,
                           float *pqx1,float *pqx0,float *pqz1,float *pqz0,
...
/************************i****************************************/
	             ...
	             for(im=0;im<mm;im++)
                      ...
                      /*  xx+=c[im]*(u1[id+im*nnz]-u1[id-(im+1)*nnz]);
                        xz+=c[im]*(u1[id+im]    -u1[id-(im+1)]);
                        zz+=c[im]*(w1[id+im]    -w1[id-(im+1)]);
                        zx+=c[im]*(w1[id+im*nnz]-w1[id-(im+1)*nnz]);*/

                        xx+=c[im]*(u1[id+im*nnz]-u1[id-(im+1)*nnz]);
                        xz+=c[im]*(u1[id+im+1]    -u1[id-(im)]);
                        zz+=c[im]*(w1[id+im]    -w1[id-(im+1)]);
                        zx+=c[im]*(w1[id+(im+1)*nnz]-w1[id-(im)*nnz]);
                       }
                     c11=vp[id]*vp[id]*(1+2*ee);
                     //c13=vp[id]*vp[id]*sqrtf(1+2*dd);
                     c13=sqrtf((2*dd*vp[id]*vp[id]+(vp[id]*vp[id]-vs[id]*vs[id]))*(vp[id]*vp[id]-vs[id]*vs[id]))-vs[id]*vs[id];
                     c33=vp[id]*vp[id];
                     c44=vs[id]*vs[id];

                      px1[id]=acoffx2[ix]*px0[id]  - acoffx1[ix]*c11*dtx*xx;
                      pz1[id]=acoffz2[iz]*pz0[id]  - acoffz1[iz]*c13*dtz*zz;
                      qx1[id]=acoffx2[ix]*qx0[id]  - acoffx1[ix]*c13*dtx*xx;
                      qz1[id]=acoffz2[iz]*qz0[id]  - acoffz1[iz]*c33*dtz*zz;
                     pqx1[id]=acoffx2[ix]*pqx0[id] - acoffx1[ix]*c44*dtx*zx;
                     pqz1[id]=acoffz2[iz]*pqz0[id] - acoffz1[iz]*c44*dtz*xz;

                      P[id]=px1[id]+pz1[id];
                      Q[id]=qx1[id]+qz1[id];
                     PQ[id]=pqx1[id]+pqz1[id];
                   }
                 }
}                      
/********************func**********************/
...
/*************func*******************/
void read_file(char FN1[],char FN2[],char FN3[],int nx,int nz,int nnx,int nnz,float *vv,float *epsilon,float *delta,float *vs,int npml)
{
		 int i,j,id;
		 ...
		 for(i=npml;i<nx+npml;i++)
		 {
			 ...
			 {
                            id=i*nnz+j;
				 fread(&vv[id],4L,1,fp1);vv[id]=3000;
...
int main(int argc,char *argv[])
{
	...
	float *u0, *u1, *px0, *qx0, *px1, *qx1, *pqx0, *pqx1;
       float *w0, *w1, *pz0, *qz0, *pz1, *qz1, *pqz0, *pqz1;
	float *P, *Q, *PQ, *shot_Dev, *shot_Hos;
...
/****************************/
        ...
        cudaMalloc(&PQ, nnz*nnx*sizeof(float));
        ...
        cudaMalloc(&pqx0, nnz*nnx*sizeof(float));   cudaMalloc(&pqx1, nnz*nnx*sizeof(float));
        cudaMalloc(&pqz0, nnz*nnx*sizeof(float));   cudaMalloc(&pqz1, nnz*nnx*sizeof(float));
...
/**********IS Loop start*******/
   for(is=1;is<=ns;is++)	
    {     
     ...
     cudaMemset(PQ, 0, nnz*nnx*sizeof(float));
     ...
     cudaMemset(pqx0, 0, nnz*nnx*sizeof(float));    cudaMemset(pqx1, 0, nnz*nnx*sizeof(float));
     cudaMemset(pqz0, 0, nnz*nnx*sizeof(float));    cudaMemset(pqz1, 0, nnz*nnx*sizeof(float));
	 ...
	 add_source<<<1,1>>>(pfac,fs,zs,nx,nz,nnx,nnz,dt,t,favg,wtype,npml,is,ds,P,Q,PQ);
        update_vel<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,npml,dt,dx,dz,u0,w0,u1,w1,P,Q,PQ,coffx1,coffx2,coffz1,coffz2);
        update_stress<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,dt,dx,dz,u1,w1,P,Q,PQ,vp,vs,npml,
                                                  px1,px0,pz1,pz0,qx1,qx0,qz1,qz0,pqx1,pqx0,pqz1,pqz0,
                                                  acoffx1,acoffx2,acoffz1,acoffz2,delta,epsilon,fs,ds,zs,is,false);
        u0=u1; w0=w1; px0=px1; pz0=pz1; qx0=qx1; qz0=qz1; pqx0=pqx1; pqz0=pqz1;

        shot_record<<<(nx+511)/512, 512>>>(nnx, nnz, nx, nz, npml, it, nt, P, shot_Dev);


           if((is==1)&&(it%400==0)&&it!=0)
            {
...
/***********free*************/ 
       ...
       cudaFree(pqx0);          cudaFree(pqx1);
       cudaFree(pqz0);          cudaFree(pqz1);
