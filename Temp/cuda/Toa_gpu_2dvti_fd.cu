...
void read_file(char FN1[],char FN2[],char FN3[],int nx,int nz,int nnx,int nnz,float *vv,float *epsilu,float *deta,int npml)
{
		 ...
		 for(i=npml;i<nx+npml;i++)
		 {
			 for(j=npml;j<nz+npml;j++)
			 {
				 ...
				 fread(&epsilu[id],4L,1,fp2);//epsilu[id]=0.;
				 fread(&deta[id],4L,1,fp3);//deta[id]=0.;
			 }
		 }
		 fclose(fp1);
		 fclose(fp2);
...
/*************wavelet\boundary**************/
        ...
        bool boolSV = true;
/********** dat document ***********/
        char FN1[100] = {"trialFD/vti_v_2000_1000.dat"};
        char FN2[100] = {"trialFD/vti_e_2000_1000.dat"};
        char FN3[100] = {"trialFD/vti_d_2000_1000.dat"};
        char FN4[250] = {"trialFD/shot.dat"};
        char FN5[250] = {"trialFD/snap.dat"};
     ...
     { 
      //if(it%500==0&&is==1)printf("---   is===%d   it===%d\n",is,it);
        ...
        update_stress<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,dt,dx,dz,u1,w1,P,Q,vp,npml,px1,px0,pz1,pz0,qx1,qx0,qz1,qz0,
                                                  acoffx1,acoffx2,acoffz1,acoffz2,deta,epsilu,fs,ds,zs,is,boolSV);
      ...
      mute_directwave<<<(nx*nt+511)/512, 512>>>(nx,nt,dt,favg,dx,dz,fs,ds,zs,is,vp,epsilu,shot_Dev,100);
