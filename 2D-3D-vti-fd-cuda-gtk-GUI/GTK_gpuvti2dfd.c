...
//###################################connect to model#########################################
void GTK2GPU_vti2dfd(GtkWidget *widget,gpointer data)
{
     run_count++;
     ...
     /* vti2dfd GPU function */
     GPU_vti2dfd(nx,nz,dx,dz,npd,SV,str_vel,str_epsilu,str_deta,favg,ns,fs,ds,zs,str_shot,str_snap,nt,dt,run_count);
...
void RunOrQuitGpu2d()
{
    ...
    // gtk_widget_set_size_request (frame, 550, 250);
     ...
     gtk_box_pack_start (GTK_BOX (vbox), hbox, TRUE, TRUE, 0);
      ...
      g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (GTK2GPU_vti2dfd), window);
...
//###################################VTI model#########################################
void GTK_gpuvti2dfd(GtkWidget *widget,gpointer data)
{
     GtkWidget *window;
     GtkWidget *button;
     GtkWidget *label;
     ...
     window=gtk_window_new(GTK_WINDOW_TOPLEVEL);
     gtk_window_set_title(GTK_WINDOW(window),"2D GPU-VTI");
     //gtk_window_set_position(GTK_WINDOW(window),GTK_WIN_POS_CENTER);
     g_signal_connect(GTK_OBJECT(window),"delete_event",GTK_SIGNAL_FUNC(gtk_widget_destroy),NULL);
...
////////////////////////////////////a////////////////////////////////////////////
     ...
     g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(RunOrQuitGpu2d),NULL);
     gtk_table_attach_defaults(GTK_TABLE(table),align,5,6,1,2);
