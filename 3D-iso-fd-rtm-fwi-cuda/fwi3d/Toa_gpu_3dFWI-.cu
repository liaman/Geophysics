...
#define npd     30   // absorbing boundry condition wield
    ...
    for (int i3 = mm ; i3 < n3 + 2*npd + mm ; i3++)
    {
        ...
        if (ltid2 < mm)
        {
          /*   tile[ithread][ithread+mm]=p1[igrid - mm*(nz+2*mm)]  */
            tile[ltid2][iter_t1]                  = p1[outIndex - mm * stride2];//iter_t1 = ltid1 + mm;
            tile[ltid2 + work2 + mm][iter_t1] = p1[outIndex + work2 * stride2];
        }
        ...
        for (int i=1; i <= mm ; i++)
        {
			c1 +=stencil[i]*(tile[t2][iter_t1-i]+ tile[t2][iter_t1+i]);//z
			c2 +=stencil[i]*(tile[t2-i][iter_t1]+ tile[t2+i][iter_t1]);//x
...
configuration requirement: <<<1,BlockSize>>> >*/
{
	...
	__shared__ float rdata[BlockSize];
    	int tid = threadIdx.x;
    	sdata[tid] = 0.0f;
	tdata[tid] = 0.0f;
	rdata[tid] = 0.0f;
...
/*< calculate nonlinear conjugate gradient >*/
{
    ...
    const int ix = blockIdx.y * blockDim.y + threadIdx.y;//0--nx's thread:ix
       ...
       for(iy=0;iy<nny;iy++)
        {
          id=iz+ix*nnz+iy*nnz*nnx;
          if(id<nnx*nny*nnz) cg[id] = -g1[id]+beta*cg[id];
        }  
}
//a#############################################################################################
...
/*< calculate temporary velocity >*/ 
{
    ...
    const int ix = blockIdx.y * blockDim.y + threadIdx.y;//0--nx's thread:ix
       ...
       for(iy=0;iy<nny;iy++)
        {
          id=iz+ix*nnz+iy*nnz*nnx;
          if(id<nnx*nny*nnz) VVtmp[id] =VV[id] + epsil*cg[id];
        }  
}
//a#############################################################################################
...
int main(int argc, char* argv[])
{
 ...
 //a######################################
	...
	char FNupdatevel[250]={"vel201202203update.dat"};
	char FNlastvel[250]={"vel201202203lastIter.dat"};
...
//a######################################
	...
	cudaMalloc(&p_derr, ns*ng*nt*sizeof(float)); 
	...
	clock_t iter_t0, iter_t1, is_t0, is_t1, ns_t0, ns_t1;
       ...
       for(iter=0; iter<niter; iter++)
		...
		cudaMemcpy(g0, g1, nnz*nnx*nny*sizeof(float), cudaMemcpyDeviceToDevice);
	       ...
	       cudaMemset(p_derr, 0, ns*ng*nt*sizeof(float));
	       ...
	       for(is=0; is<ns; is++)
	            ...
	            for(it=0; it<nt; it++)
	             ...
	             cudaMemcpy(p_obs, p_IO, ng*nt*sizeof(float), cudaMemcpyHostToDevice);
                    ...
                    cuda_com2derr<<<dimt, dimb>>>(p_com, &p_derr[is*ng*nt], nx, ny, nt);
		      cudaMemcpy(&obj, &pars[0], sizeof(float), cudaMemcpyDeviceToHost);
	             ...
	             for(it=nt-1; it>-1; it--)
                     ...
                     is_t1=clock();
                     if(is%5==0)printf("\n###   IS:  ");
                     printf(" (%2d)%.2f(min);",is,((float)(is_t1-is_t0))/60000000.0);
	       }//IS loop end
              ns_t1=clock();
		...
		/* calculate the factor beta in conjugate gradient method */
              if (iter>0) cuda_cal_beta<<<1, BlockSize>>>(&pars[1], g0, g1, cg, nnx*nny*nnz);
	      ...
	      for(is=0; is<ns; is++)
               {   
	            ...
	            for(it=0; it<nt; it++)
                    ...
                    cuda_sum_alpha12<<<dimt, dimb>>>(alpha1, alpha2, p_cal, p_obs, &p_derr[is*ng*nt], nx, ny, nz, nt);
              }//is loop end
		...
		if(iter==0) 	{obj1=obj; objval[iter]=1.0;}
              ...
              iter_t1=clock();
              printf("###   obj=%f, beta=%f, epsil=%.2f, alpha=%.2f : %.2f(min)\n",
