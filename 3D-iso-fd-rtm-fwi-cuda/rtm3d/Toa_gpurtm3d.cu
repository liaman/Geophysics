/*<>*/
/*              3D-finite different RTM          */

/*  Initial Code in 2014, Xi'an Jiaotong University (Pengliang Yang) */

/*  Ps :
             absorb boundary(mm-npd-nx-npd-mm), 
...
int main(int argc, char* argv[])
{
	...
	float *v0, *vv, *d_wlt, *d_vv, *d_p0, *d_p1, *ptr, *d_g0, *d_g1, v_mu;
       float *p_cal_it, *p_cal_nt, *p_cal_out, *d_bndr;
       float *g_is, *g_ns, *source;


	sf_file Fv, Fw, Fshot, Fg;
	...
	sf_putint(Fshot,"n3",ny);
    	...
    	p_cal_out=(float*)malloc(ng*nt*sizeof(float));
	...
	for(is=0; is<ns; is++){
	  ...
	  for(it=0; it<nt; it++){
	    ...
	    ptr=d_p0; d_p0=d_p1; d_p1=ptr;
           ...
           //cuda_free_surface<<<dimg,dimb>>>(d_p0, d_p1, nz, nx, ny);
            ...
            /*< record & (xy)2(txy) >*/
	    cuda_record<<<(ng+511)/512, 512>>>(d_p0, p_cal_it, d_gzxy, ng);
           cuda_trans_xy2txy<<<(ng+511)/512, 512>>>(p_cal_it, p_cal_nt, it, nt, ng);
	 ...
	 /*   if(is==0&&it==kt){
	  ...
	  cudaMemcpy(p_cal_out, p_cal_nt, ng*nt*sizeof(float), cudaMemcpyDeviceToHost);
	  sf_floatwrite(p_cal_out, ng*nt, Fshot);
	  ...
	  for(it=nt-1; it>-1; it--){
	    ...
	    cuda_step_fd3d<<<dimg,dimb>>>(d_g0, d_g1, d_vv, _dz2, _dx2, _dy2, nz, nx, ny);
       ...
       /*    if(is==0&&it==kt){
        ...
        /*   if(is==0&&it%300==0){
	      ...
	      sf_floatwrite(v0, nz*nx*ny, Fw);	  
           ...
           cuda_cal_gradient<<<dimg,dimb>>>(g_is, d_p1, d_g1, nz, nx, ny);
