...
#include"src_seismic_RungeKutta.h"
...
{
	int nx,nz,nt,ix,iz,irs,nxf,nxe,nzf,nze,ixo,izo,i,NRS,cntt,isx,ia,nxo,nzo,nsx,na,iray,id,flag_selextfile;
	...
	/* ensure takeoff point is within model */
        ...
        float *raypath_nray_xz_nt;
        raypath_nray_xz_nt=alloc1float(nsx*na*2*nt);
	/* allocate workspace */
		...
		for(ia=0;ia<na;ia++) {
			...
			a=fa+ia*da;
        		...
        		ft=0;/* Assume 0s as initial time */
                     if(ia%20==0) printf("isx=%2d, ia=%3d, angle = %f\n",isx,ia,a);
			...
			for(cntt=0;cntt<nt-1;cntt++)
                        {
                        id = iray*2*nt + cntt;
                        if(fabs(raypath_nray_xz_nt[id]-raypath_nray_xz_nt[id+1])>10.0){
                                for(i=cntt;i<nt;i++){
                                        raypath_nray_xz_nt[id]=0.0;
                                        }
                                break;
                                }
                        if(fabs(raypath_nray_xz_nt[id+nt]-raypath_nray_xz_nt[id+nt+1])>10.0){
                                for(i=cntt;i<nt;i++){
                                        raypath_nray_xz_nt[id+nt]=0.0;
                                        }
                                break;
                                }
                        }
